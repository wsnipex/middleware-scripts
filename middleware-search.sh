...
function check_versions {
  ...
  case $process in
    apache|httpd)
      ap_ld_path=$(dirname $command)/../lib
    ...
    tomcat)
      ...
      if [ ! -x "$jps" ]; then
        ...
        [ -x "$jps" ] && catalina_home=$(${jps} -lv | $GREP $pid | $SED 's/^.*-Dcatalina.home=\(.*\) .*$/\1/g')
      fi
        ...
        for p in ${classpath}; do
          if $(echo "$p" | $GREP -v Main | $GREP -qi tomcat) ; then
            ...
            if [ -d $tctmp ] || [ -f $tctmp ]; then
              catalina_home_t=$(echo $(dirname $tctmp) | $SED -e 's/bin//g' -e 's/lib//g' | sort -u)
      ...
      tomcat_command="CATALINA_HOME=${catalina_home} JAVA_HOME=${java_home} sh ${catalina_home}/bin/catalina.sh version"
    ...
    jboss)
      ...
      jboss_home=$(${jinfo} $pid 2>&1 | $GREP jboss.home.dir | cut -d " " -f 3)
      if [ -z "${jboss_home}" ]; then
        [ $DEBUG ] && echoerr "INFO failed to detect JBOSS_HOME - trying classpath..."
        classpath=$($PS | $GREP $pid | $SED -e 's/^.*-classpath \(.*\) .*$/\1/g' -e 's/:/ /g')
      ...
      jboss_command="JBOSS_HOME=${jboss_home} JAVA_HOME=${java_home} sh ${jboss_home}/bin/run.sh -V"
      ...
      ;;
    websphere)
      ws_home=$(echo $command | $SED 's|\(.*AppServer\).*$|\1/bin|')
